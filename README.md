...
-   **Editais PIBIC:** Caso o projeto seja submetido a edital PIBIC, atente-se para não incluir qualquer informação que permita a identificação dos/das proponentes, inclusive na capa ou no nome do arquivo, como o nome do(da) estudante ou nome do orientador(a), mesmo que incompletos. O template `modeloProjetoIC.typ` está configurado para auxiliar nisso.
...
- **Referências Bibliográficas:** O Typst suporta diversos formatos de bibliografia, incluindo BibTeX (`.bib`) e [Hayagriva](https://github.com/typst/hayagriva) (`.yml`). Este repositório utiliza o formato YAML do Hayagriva por ser mais legível e moderno. Para detalhes sobre como organizar suas referências em YAML, consulte a [documentação do formato Hayagriva](https://github.com/typst/hayagriva/blob/main/docs/file-format.md).